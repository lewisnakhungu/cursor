...
async function main(): Promise<void> {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      // Catch rows that may have been created before tenantId column existed
      ...
      console.log(`  StockBatch rows updated: ${batches.count} (+ ${orphanBatches} orphan fix)`);
      console.log(`  Sale rows updated: ${sales.count} (+ ${orphanSales} orphan fix)`);
      console.log(`  SaleLine rows updated: ${lines.count} (+ ${orphanLines} orphan fix)`);
    });
  } finally {
