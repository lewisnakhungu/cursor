...
async function getTopDrugs(from: Date, limit: number): Promise<TopSellingDrug[]> {
...
function buildSalesByDay(from: Date): Promise<SalesByDayRow[]> {
  return prisma.sale
    .findMany({
...
export async function getSalesReport(
  ...
  return runAction("getSalesReport", async () => {
    ...
    return {
      reportTitle:
        periodDays === 7 ? "Weekly Sales & Stocking Report" : "Monthly Sales & Stocking Report",
      periodDays,
      periodLabel,
      ...
      salesByDay: await buildSalesByDay(since),
